...
BINS_SPACE = {
    "x":         np.linspace(-2.5,  2.5,  30, dtype=np.float32),
    "x_dot":     np.linspace(-5.0,  5.0,  30, dtype=np.float32),
    "theta":     np.linspace(-0.25, 0.25, 30, dtype=np.float32),
    "theta_dot": np.linspace(-5.0,  5.0,  30, dtype=np.float32),
}
...
def evaluate(pi: CartPoleCuda, n_episodes: int = 5, render: bool = False) -> None:
